...
public class MajCompController implements Initializable {
    competition target;
    ...
    public MajCompController(competition target) throws SQLException, ClassNotFoundException {
        this.target = target;
    }
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        /*--------------------------------------------------------------------------------------------------------------
        *Init des element de la page
        * TODO:
        *  - preremplire les champs
        *  - init les tableaux
        *  - selcetioner le bon tableau au départ
        * ------------------------------------------------------------------------------------------------------------*/
        if(target.isENEQUIPE()){
            cb_equipe.setSelected(true);
        }
        ...
        try {
            obsER.addAll(bdd.listCandidatE(target.getIDCOMPETITION()));
        ...
        try {
            obsPR.addAll(bdd.listCandidatP(target.getIDCOMPETITION()));
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
            ...
            public void handle(ActionEvent actionEvent) {
                try {
                    ...
                    if (cb_equipe.isSelected()) {/* comp equipe*/
                        ...
                        for(equipe e:listCanEqu){
                            listid.add(e.getIDEQUIPE());
                        }
                        bdd.majCompE(id,txt_nom.getText(),txt_date.getText(),cb_equipe.isSelected(),listid);
                        listid.clear();

                    } else { /* comp Personne */
                        ArrayList<Integer> listid = new ArrayList<>();
                        for(personne e:listCanPer){
                            listid.add(e.getIDPERSONNE());
                        }
                        bdd.majCompE(id,txt_nom.getText(),txt_date.getText(),cb_equipe.isSelected(),listid);
                        listid.clear();
                    }
